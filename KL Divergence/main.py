...
def kl_divergence(P, Q):
    ...
    """
    # Ensure the distributions are normalized (sum to 1) to represent valid probability distributions
    P = np.array(P, dtype=np.float64)
    Q = np.array(Q, dtype=np.float64)

    # Replace zeros in Q to avoid division by zero errors (KL Divergence is undefined if Q[i] == 0)
    Q = np.where(Q == 0, 1e-10, Q)

    # Calculate KL Divergence using the formula: sum(P[i] * log(P[i] / Q[i]))
